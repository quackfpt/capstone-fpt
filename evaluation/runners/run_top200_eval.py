...
DATASET_DIR = EVAL_DIR / "external_datasets" / "GPTScan-Top200"
OUTPUT_FILE = EVAL_DIR / "report_result" / "top200_evaluation_results.json"
CHECKPOINT_FILE = EVAL_DIR / "runners" / "top200_checkpoint.json"
...
def run_evaluation(resume=False):
    ...
    for i, contract in enumerate(contracts):
        ...
        # Console: compact pipeline output per contract
        print_pipeline_result(i+1, total, contract, result, elapsed, verdict, status, detected_types)

        # JSON: full pipeline details for reviewers
        pipeline = extract_pipeline_details(result)
