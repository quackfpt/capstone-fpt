...
class SolidityASTParser:
    ...
    def _parse_with_tree_sitter(self, code: str, version: str) -> ASTResult:
        ...
        # Resolve inherited state variables and re-evaluate has_state_change.
        # Initial parse only sees state vars declared in the contract itself,
        # missing inherited vars from parent contracts (e.g., balances from Parent).
        self._resolve_inherited_state_vars(contracts, code)

        # Propagate has_external_call / has_state_change through internal calls.
        # Must run AFTER inheritance resolution (needs accurate state_change data).
        self._resolve_cross_function_calls(contracts)
    ...
    def _resolve_inherited_state_vars(self, contracts: list, code: str) -> None:
        """Resolve inherited state variables and re-evaluate has_state_change.

        For each contract, collect state variable names from all in-file parent
        contracts (transitive). Then re-check every function's has_state_change
        against the full set of state variable names.

        This fixes the case where a child contract modifies a state variable
        declared in a parent contract (e.g., `balances[msg.sender] = 0` where
        `balances` is declared in a parent).

        Only resolves parents defined in the same file — external parents
        (imported from other files) are not available to tree-sitter.
        """
        # Build name → contract lookup (skip synthetic contracts like "(free functions)")
        name_to_contract = {}
        for c in contracts:
            if c.contract_type not in ("free",):
                name_to_contract[c.name] = c

        # For each contract, collect all inherited state var names (transitive, deduped)
        # Uses iterative BFS to handle multi-level inheritance (A is B, B is C)
        for contract in contracts:
            if contract.contract_type in ("interface", "library", "free"):
                continue  # interfaces/libraries don't have inheritable state

            # BFS: collect state var names from all ancestors
            all_state_var_names = set(sv.name for sv in contract.state_variables)
            visited = {contract.name}
            queue = list(contract.inheritance)

            while queue:
                parent_name = queue.pop(0)
                if parent_name in visited:
                    continue  # avoid cycles in diamond inheritance
                visited.add(parent_name)

                parent = name_to_contract.get(parent_name)
                if parent:
                    for sv in parent.state_variables:
                        all_state_var_names.add(sv.name)
                    # Also traverse grandparents
                    queue.extend(parent.inheritance)

            # Only re-evaluate if we found additional inherited vars
            own_var_names = set(sv.name for sv in contract.state_variables)
            if all_state_var_names == own_var_names:
                continue  # no new vars from inheritance

            all_var_list = list(all_state_var_names)
            for func in contract.functions:
                if func.mutability in ("view", "pure"):
                    continue  # compiler guarantees no state change
                new_has_state_change = self._has_state_change(func.code, all_var_list)
                if new_has_state_change and not func.has_state_change:
                    func.has_state_change = True

    def _resolve_cross_function_calls(self, contracts: list) -> None:
        """Propagate has_external_call and has_state_change through internal calls.

        If function A calls function B (same contract), A inherits B's properties:
        - If B has_external_call → A has_external_call
        - If B has_state_change → A has_state_change

        This detects cross-function reentrancy patterns like:
            withdraw() calls _send() (external call) and _reset() (state change)
            → withdraw should be flagged as potential reentrancy

        Uses iterative propagation to handle call chains (A→B→C).
        Only traces calls within the same contract (not cross-contract).
        """
        for contract in contracts:
            if contract.contract_type in ("interface", "library", "free"):
                continue

            func_map = {f.name: f for f in contract.functions}
            if not func_map:
                continue

            # Build call graph: for each function, find which other functions it calls
            call_graph = {}
            for func in contract.functions:
                callees = set()
                # Strip comments and strings to avoid false matches
                # e.g., "// does NOT call reset()" should not match reset()
                code_no_comments = re.sub(r'//[^\n]*', '', func.code)
                code_no_comments = re.sub(r'/\*.*?\*/', '', code_no_comments, flags=re.DOTALL)
                code_no_comments = re.sub(r'"(?:[^"\\]|\\.)*"', '""', code_no_comments)
                code_no_comments = re.sub(r"'(?:[^'\\]|\\.)*'", "''", code_no_comments)

                for other_name in func_map:
                    if other_name == func.name:
                        continue
                    # Match function calls: _reset(), _send(amt), etc.
                    # Use word boundary to avoid matching substrings
                    if re.search(rf'\b{re.escape(other_name)}\s*\(', code_no_comments):
                        callees.add(other_name)
                call_graph[func.name] = callees

            # Iterative propagation: keep going until no changes
            # (handles chains like A→B→C where C has the external call)
            changed = True
            iterations = 0
            max_iterations = len(func_map) + 1  # guaranteed to converge
            ...
            while changed and iterations < max_iterations:
                ...
                for func in contract.functions:
                    if func.mutability in ("view", "pure"):
                        continue
                    for callee_name in call_graph.get(func.name, set()):
                        callee = func_map.get(callee_name)
                        if not callee:
                            continue
                        if callee.has_external_call and not func.has_external_call:
                            func.has_external_call = True
                            changed = True
                        if callee.has_state_change and not func.has_state_change:
                            func.has_state_change = True
                            changed = True
    ...
    def _parse_with_regex(self, code: str) -> ASTResult:
        ...
        # Resolve inherited state variables (same logic as tree-sitter path)
        self._resolve_inherited_state_vars(contracts, code)

        # Propagate through internal calls (same logic as tree-sitter path)
        self._resolve_cross_function_calls(contracts)
