...
QDRANT_DB_PATH = os.getenv("QDRANT_DB_PATH", "./qdrant_db_forge")
...
async def analyze_contract(
    ...
    Query params (for ablation study):
        disable_rag=true    Skip RAG search + reranking (conditions A, C)
        disable_slither=true  Skip Slither analysis (conditions A, B)
    """
    ablation_mode = ""
    if disable_rag and disable_slither:
        ablation_mode = "LLM-only"
    elif disable_rag:
        ablation_mode = "LLM+Slither"
    elif disable_slither:
        ablation_mode = "LLM+RAG"

    print(f"\n{'='*60}")
    print(f"[PIPELINE v7.0] 6-Step Analysis{f' (ABLATION: {ablation_mode})' if ablation_mode else ''}")
    print(f"{'='*60}")
    ...
    try:
        ...
        print(f"\n[STEP 2+3/6] Slither{'(DISABLED)' if disable_slither else ''} + RAG{'(DISABLED)' if disable_rag else ''} Search (parallel)...")
        ...
        # Run Slither + RAG in parallel (respecting disable flags)
        async def run_slither():
            if disable_slither:
                return ["No Slither analysis available (disabled for ablation study)"]
            return await asyncio.to_thread(slither.get_warnings_for_ai, code_text)

        slither_warnings, rag_candidates = await asyncio.gather(
            run_slither(), run_rag_search()
        )
        ...
        HINT_TO_VULN = {
            ...
            "controlled-array-length": "Integer Overflow",
        ...
        if risky_functions:
            ...
            # Intentionally exclude raw code snippet: cross-encoder rerank-2.5
            # matches on structural similarity, so any 'function withdraw(uint256 amount)'
            # snippet got pulled toward a mislabeled KB entry (keep-tecdsa withdraw
            # tagged SWC-101 despite being a reentrancy-adjacent withdraw pattern).
            # Dropping Code: recovered correct top-1 on Reentrancy + Unchecked Return.
            rerank_query = (
        ...
        response = {
            ...
            "ablation": ablation_mode if ablation_mode else "full",
            ...
            "rag_findings": {
                ...
                # Score distribution for CRAG threshold validation
                "score_distribution": {
                    "relevance_scores": [round(c.get('relevance_score', 0), 4) for c in reranked_results],
                    "bi_encoder_scores": [round(c.get('bi_encoder_score', c.get('similarity', 0)), 4) for c in reranked_results],
                    "max_relevance": round(max((c.get('relevance_score', 0) for c in reranked_results), default=0), 4),
