...
The following rules are derived from 21,032 real audit cases in our knowledge base.
    ...
    def create_advanced_prompt(
        ...
        if slither_warnings and len(slither_warnings) > 0:
            ...
            if first_warning.startswith('⚠️ SLITHER UNAVAILABLE'):
                ...
                # Check if protections were stripped for compilation
                stripped_note = [w for w in slither_warnings if "NOTE: Original code uses" in w]
                if stripped_note:
                    slither_section += "**WARNINGS FOUND (but security modifiers were stripped for compilation):**\n"
                    slither_section += (
                        "**CRITICAL CONTEXT:** The following Slither warnings were generated AFTER removing "
                        "security modifiers (e.g., nonReentrant, onlyOwner, Pausable) for compilation. "
                        "Some or ALL of these warnings may be FALSE POSITIVES because the original code "
                        "HAS these protections. You MUST verify each warning against the ORIGINAL code above "
                        "(which still contains the modifiers). If the original code has ReentrancyGuard/nonReentrant, "
                        "do NOT report reentrancy. If it has onlyOwner, do NOT report access control issues.\n\n"
                    )
                    ...
                    for warning in slither_warnings:
                        slither_section += f"  {warning}\n"
                    slither_section += "\n PAY SPECIAL ATTENTION to these line numbers!\n"
...
"""
    ...
    def _filter_pragma_080(self, analysis_json: dict, solidity_version: str) -> dict:
        ...
        # Parse version from pragma (e.g., "^0.8.26", ">=0.8.0", "0.8.4", "0.8")
        # Support both 3-part (0.8.26) and 2-part (0.8) version strings
...
"""
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # temperature=0 for deterministic output (reproducible evaluation)
                ...
                if "429" in error_msg or "quota" in error_msg.lower() or "rate" in error_msg.lower():
                    # Rate limit: parse API-suggested delay or use exponential backoff
